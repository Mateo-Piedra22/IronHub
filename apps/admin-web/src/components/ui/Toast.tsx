...
export function useToast() {
  ...
  const error = useCallback(
    (message: string, options?: { duration?: number; action?: Toast["action"] }) =>
      toast(message, "error", { duration: 8000, ...options }),
    [toast]
  );
  const warning = useCallback(
    (message: string, options?: { duration?: number; action?: Toast["action"] }) =>
      toast(message, "warning", { duration: 6000, ...options }),
    [toast]
  );
  ...
  return { toast, success, error, warning, info, dismiss: removeToast, clearAll };
}
