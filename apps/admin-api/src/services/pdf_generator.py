...
class PDFGenerator:
    ...
    def generar_recibo(
        ...
        info_style = ParagraphStyle('InfoRight', parent=styles['Normal'], alignment=TA_RIGHT)
        ...
        if detalles and len(detalles) > 0:
            for det in detalles:
                desc = det.get('descripcion') or det.get('concepto_nombre') or 'Concepto'
        ...
        details_table = Table(details_data, colWidths=[3.5*inch, 1*inch, 1.5*inch, 1.5*inch])
