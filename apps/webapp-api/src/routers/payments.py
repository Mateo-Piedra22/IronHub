...
async def api_metodos_pago_update(
    ...
    try:
        ...
        if "nombre" in payload:
            data['nombre'] = (payload.get("nombre") or existing.nombre or "").strip() or existing.nombre
        if "icono" in payload:
            data['icono'] = payload.get("icono")
        if "color" in payload:
            data['color'] = (payload.get("color") or existing.color or "#3498db").strip()
...
async def api_tipos_cuota_catalogo(
    ...
    try:
        ...
        tipos = sorted(tipos, key=lambda t: (
...
async def api_tipos_cuota_delete(
    ...
    try:
        ...
        if not deleted:
            raise HTTPException(status_code=404, detail="No se pudo eliminar el tipo de cuota")
        return {"ok": True}
    except HTTPException:
        raise
    except Exception as e:
...
async def api_pagos_create(
    ...
    try:
        ...
        if isinstance(conceptos_raw, list) and len(conceptos_raw) > 0:
            ...
            try:
                pago_id = svc.registrar_pago_avanzado(usuario_id, metodo_pago_id_int, conceptos, fecha_dt)
            except ValueError as ve:
                raise HTTPException(status_code=400, detail=str(ve))
            except Exception as e:
        ...
        try:
            pago_id = svc.registrar_pago(usuario_id, monto, mes, año, metodo_pago_id_int)
        except ValueError as ve:
            raise HTTPException(status_code=400, detail=str(ve))
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
...
async def api_pagos_update(
    ...
    try:
        ...
        if advanced_conceptos:
            ...
            try:
                svc.modificar_pago_avanzado(int(pago_id), usuario_id, metodo_pago_id_int, conceptos, fecha_dt)
            except ValueError as ve:
                raise HTTPException(status_code=400, detail=str(ve))
            except Exception as e:
                raise HTTPException(status_code=500, detail=str(e))
