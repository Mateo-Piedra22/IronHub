...
def _sign_excel_view(
    ...
    try:
        ...
        tenant = ""
    try:
        base = f"{tenant}|{int(rutina_id)}|{int(weeks)}|{filename}|{int(ts)}|{qr}|{sh}".encode(
            "utf-8"
        )
    except Exception:
...
def _sign_excel_view_draft_data(
    data: str,
    weeks: int,
    filename: str,
    ts: int,
...
def _build_excel_export_filename(
    nombre_rutina: str, dias: Any, usuario_nombre: str
) -> str:
    try:
...
@router.get(
    "/api/rutinas/{rutina_id}/export/excel",
    dependencies=[
        Depends(require_feature("rutinas")),
        Depends(require_scope_gestion("rutinas:read")),
    ],
...
async def api_rutina_export_excel(
    ...
    try:
        ...
        excel_filename = _sanitize_download_filename(
            filename, f"rutina_{rutina_id}", ".xlsx"
        )
        return FileResponse(
            xlsx_path,
...
@router.get(
    "/api/rutinas/{rutina_id}/export/excel_view_url",
    dependencies=[
        Depends(require_feature("rutinas")),
        Depends(require_scope_gestion("rutinas:read")),
    ],
...
async def api_rutina_excel_view_url(
    ...
    try:
        ...
        if not tenant:
            raise HTTPException(
                status_code=400, detail="Tenant requerido para previsualización"
            )
        ...
        # Build filename
        if not filename:
            nombre_rutina = rutina_data.get("nombre_rutina") or rutina_data.get(
                "nombre", f"rutina_{rutina_id}"
            )
        ...
        # Generate signature
        ts = int(time.time())
        sig = _sign_excel_view(
            rutina_id, weeks, base_name, ts, qr_mode=qr_mode, sheet=sheet_norm
        )
...
@router.get(
    "/api/rutinas/{rutina_id}/export/pdf_view_url",
    dependencies=[
        Depends(require_feature("rutinas")),
        Depends(require_scope_gestion("rutinas:read")),
    ],
...
@router.get(
    "/api/rutinas/{rutina_id}/export/excel_view.xlsx",
    dependencies=[
        Depends(require_feature("rutinas")),
        Depends(require_scope_gestion("rutinas:read")),
    ],
...
async def api_rutina_excel_view(
    ...
    try:
        ...
        if abs(now - int(ts)) > 600:
            raise HTTPException(
                status_code=403, detail="Link de previsualización expirado"
            )
    except HTTPException:
        raise
    except Exception:
    ...
    # Verify signature
    expected = _sign_excel_view(
        rutina_id, weeks, base_name, int(ts), qr_mode=qr_mode, sheet=sheet_norm
    )
    ...
    try:
        ...
        if not factory:
            raise HTTPException(
                status_code=503,
                detail=f"Database connection unavailable for tenant '{t}'",
            )
        session = factory()
        svc = TrainingService(session)
        ...
        tmp_fd, tmp_path = tempfile.mkstemp(
            prefix=f"rutina_view_{rutina_id}_", suffix=".xlsx"
        )
        try:
            os.close(tmp_fd)
        except Exception:
...
@router.get(
    "/api/rutinas/{rutina_id}/export/pdf_view.pdf",
    dependencies=[
        Depends(require_feature("rutinas")),
        Depends(require_scope_gestion("rutinas:read")),
    ],
...
@router.post(
    "/api/rutinas/export/draft_pdf_url",
    dependencies=[
        Depends(require_feature("rutinas")),
        Depends(require_scope_gestion("rutinas:read")),
    ],
...
async def render_draft_excel(
    ts: int, data: str, sig: str, filename: Optional[str] = None, rm=Depends(get_rm)
):
    """
        ...
        for ej in ejercicios_list[:200]:
            ...
            try:
                setattr(
                    re,
                    "nombre_ejercicio",
                    ej.get("nombre_ejercicio") or ej.get("nombre") or None,
                )
            except Exception:
                pass
            try:
                setattr(
                    re, "notas", str(ej.get("notas") or ej.get("nota") or "").strip()
                )
            except Exception:
                pass
