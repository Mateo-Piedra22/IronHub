...
class PDFGenerator:
    def __init__(self, branding_config=None):
        ...
        self.output_dir_rutinas = ensure_writable_dir(
            pref_rutinas, "rutinas_exportadas"
        )

        # Usar logo del sistema de branding si está disponible
        self.branding_config = branding_config or {}
        self.logo_path = self.branding_config.get("main_logo_path") or os.path.join(
            "assets", "gym_logo.png"
        )
        self.logo_url = (
        ...
        try:
            self.gym_name = str(
                (
                    self.branding_config.get("gym_name")
                    or get_gym_name("Gimnasio")
                    or "Gimnasio"
                )
            )
        except Exception:
        ...
        try:
            self.gym_address = str(
                (self.branding_config.get("gym_address") or "Saavedra 2343, Santa Fe")
            )
        except Exception:
    ...
    def generar_recibo(
        ...
        detalles_override: Optional[List[Dict]] = None,
        mostrar_logo: Optional[bool] = True,
        mostrar_metodo: Optional[bool] = True,
        mostrar_dni: Optional[bool] = True,
        ...
        header_bg_color = colors.HexColor(
            self._get_dynamic_color("alt_background_color", "#111827")
        )
        body_bg_color = colors.HexColor(
            self._get_dynamic_color("table_body_color", "#FFFFFF")
        )
        border_color = colors.HexColor(
            self._get_dynamic_color("border_color", "#E5E7EB")
        )
        muted_text_color = colors.HexColor(
            self._get_dynamic_color("muted_text_color", "#6B7280")
        )
        ...
        right_info_style = ParagraphStyle(
            name="RightInfo", parent=small_style, alignment=TA_RIGHT
        )
        right_info_para = Paragraph(
            f"<b>Comprobante</b> N° {recibo_numero}<br/><b>Fecha</b> {fecha_str_disp}",
            right_info_style,
        )
                    ...
                    try:
                        ...
                        if r.status_code == 200 and r.content:
                            ...
                            try:
                                ...
                                ext = ".png"
                            tmp_logo_path = os.path.join(
                                tempfile.gettempdir(),
                                f"ironhub_logo_{int(time.time())}{ext}",
                            )
        ...
        header_table = Table(
            header_data, colWidths=[1.7 * inch, 4.3 * inch, 1.5 * inch]
        )
        ...
        try:
            ...
            if hasattr(pago, "metodo_pago_nombre") and getattr(
            ...
            # Si tenemos el ID del método, intentar resolver el nombre desde la base de datos
                ...
                try:
                    ...
                    with db.get_connection_context() as conn:
                        cur = conn.cursor()
                        cur.execute(
                            "SELECT nombre FROM metodos_pago WHERE id = %s",
                            (pago.metodo_pago_id,),
                        )
                        r = cur.fetchone()
                        if r:
        ...
        try:
            ...
            dni_disp = (
                usuario_dni
                if usuario_dni is not None
                else getattr(usuario, "dni", None)
            )
            ...
            tipo_cuota_disp = (
                tipo_cuota or getattr(usuario, "tipo_cuota", None) or "No especificado"
            )
            ...
            if mostrar_metodo is not False:
                info_rows.append(
                    ["Método de Pago", (metodo_nombre or "No especificado")]
                )
        ...
        if (
            detalles_override
            and isinstance(detalles_override, list)
            and len(detalles_override) > 0
        ):
            for det in detalles_override:
                try:
                    desc = str(
                        det.get("descripcion") or det.get("concepto") or "Concepto"
                    )
            ...
            pago_details_data.append(
                [
                    f"Cuota mensual: {mes_nombre} {pago.año}",
                    "1",
                    monto_formateado,
                    monto_formateado,
                ]
        ...
        pago_table = Table(
            pago_details_data, colWidths=[3.5 * inch, 1 * inch, 1.5 * inch, 1.5 * inch]
        )
        pago_table.setStyle(
            TableStyle(
                [
                    ...
                    subtotal_val = sum(
                        [
                            (
                                ...
                                * float(
                                    d.get("precio_unitario") or d.get("precio") or 0
                                )
                            )
                            for d in detalles_override
                        ]
                    )
                except Exception:
        ...
        try:
            if observaciones or emitido_por:
                ...
                notes_table = Table(
                    [[header_notes], [notes_para]], colWidths=[7.5 * inch]
                )
    ...
    def _footer(self, canvas, doc):
        try:
            ...
            footer_color = colors.HexColor(
                self._get_dynamic_color("alt_background_color", "#434C5E")
            )
    ...
    def generar_pdf_rutina(
        self, rutina: Rutina, usuario: Usuario, exercises_by_day: dict
    ) -> str:
        ...
        user_style = ParagraphStyle(
            name="UserStyle", parent=styles["h2"], alignment=TA_CENTER
        )
        ...
        elements.append(
            Paragraph(f"Plan de Entrenamiento para: {usuario.nombre}", user_style)
        )
        ...
        for day in sorted(exercises_by_day.keys()):
            ...
            for ej in ejercicios_del_dia:
                table_data.append(
                    [
                        ...
                        Paragraph(
                            ej.ejercicio.grupo_muscular or "N/A", styles["Normal"]
                        ),
            ...
            table = Table(
                table_data, colWidths=[2.5 * inch, 2 * inch, 1 * inch, 2 * inch]
            )
                ...
                TableStyle(
                    [
                        (
                            ...
                            (0, 0),
                            (-1, 0),
                            colors.HexColor(header_bg_color),
                        ),
                        ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke),
