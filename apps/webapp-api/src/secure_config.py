...
class SecureConfig:
    ...
    def get_env_variable(
        cls, key: str, default: Any = None, required: bool = False
    ) -> Any:
        val = os.getenv(key)
        if val is None:
            if required:
    ...
    def verify_owner_password(cls, password: str) -> bool:
        ...
        if sh.startswith("$2"):
            try:
                ...
                return bool(
                    bcrypt.checkpw(password.encode("utf-8"), sh.encode("utf-8"))
                )
            except Exception:
                return False
        return sh == password
    ...
    def require_waba_encryption(cls) -> None:
        ...
        dev_mode = cls.get_env_bool("DEVELOPMENT_MODE", False) or env in (
            "dev",
            "development",
            "local",
        )
        ...
        if cls._is_weak_waba_key(k) or cls._get_fernet() is None:
            raise RuntimeError(
                "WABA_ENCRYPTION_KEY débil o ausente (requerido en producción)"
            )
