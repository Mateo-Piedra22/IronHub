...
class WhatsappRepository(BaseRepository):
    ...
    def crear_notificacion_cupo_completa(
        self,
        usuario_id: int,
    ...
    def registrar_mensaje_whatsapp(
        self,
        user_id: int,
        message_type: str,
        template_name: str,
    ...
    def verificar_mensaje_enviado_reciente(
        self, user_id: int, message_type: str, horas_limite: int = 24
    ) -> bool:
    ...
    def obtener_historial_mensajes_whatsapp(
        self, user_id: int = None, message_type: str = None, limit: int = 100
    ) -> List[Dict]:
        """Obtiene el historial de mensajes WhatsApp"""
        stmt = (
            select(WhatsappMessage, Usuario)
            .outerjoin(Usuario)
        ...
        return [
            {
                ...
                "template_name": wm.template_name,
                "phone_number": wm.phone_number,
                "message_id": wm.message_id,
                "sent_at": wm.sent_at,
                "status": wm.status,
                "message_content": wm.message_content,
    ...
    def _enviar_mensaje_bienvenida_automatico(
        self, usuario_id: int, nombre: str, telefono: str
    ):
            ...
            ultimo_mes = (
                self.db.scalar(
                    select(func.count(WhatsappMessage.id)).where(
                        ...
                        > self._now_utc_naive() - timedelta(days=30)
                    )
                )
                or 0
            )
    ...
    def limpiar_mensajes_antiguos_whatsapp(self, dias_antiguedad: int = 90) -> int:
        stmt = delete(WhatsappMessage).where(
            WhatsappMessage.sent_at
            < self._now_utc_naive() - timedelta(days=dias_antiguedad)
        )
        result = self.db.execute(stmt)
        self.db.commit()
        return result.rowcount

    def contar_mensajes_whatsapp_periodo(
        self,
        user_id: int = None,
        telefono: str = None,
        ...
        if start_time:
            stmt = stmt.where(WhatsappMessage.sent_at >= start_time)
        if end_time:
            stmt = stmt.where(WhatsappMessage.sent_at <= end_time)
    ...
    def obtener_ultimo_mensaje_whatsapp(
        self,
        user_id: int = None,
        telefono: str = None,
    ...
    def obtener_mensaje_whatsapp_por_pk(
        self, user_id: int, pk_id: int
    ) -> Optional[Dict]:
        wm = self.db.scalar(
            select(WhatsappMessage).where(
                WhatsappMessage.id == pk_id, WhatsappMessage.user_id == user_id
            )
        )
        if wm:
            return {
    ...
    def obtener_mensaje_whatsapp_por_message_id(
    ...
    def obtener_telefonos_con_mensajes_fallidos(
        self, fecha_limite: datetime
    ) -> List[str]:
        stmt = select(func.distinct(WhatsappMessage.phone_number)).where(
            WhatsappMessage.status == "failed", WhatsappMessage.sent_at >= fecha_limite
        )
        return list(self.db.scalars(stmt).all())

    def actualizar_estado_mensaje_whatsapp(
        self, message_id: str, nuevo_estado: str
    ) -> bool:
        stmt = (
            update(WhatsappMessage)
            .where(WhatsappMessage.message_id == message_id)
            .values(status=nuevo_estado)
        )
        result = self.db.execute(stmt)
        self.db.commit()
    ...
    def eliminar_mensaje_whatsapp_por_message_id(
        self, user_id: int, message_id: str
    ) -> bool:
        stmt = delete(WhatsappMessage).where(
            WhatsappMessage.message_id == message_id, WhatsappMessage.user_id == user_id
        )
        result = self.db.execute(stmt)
        self.db.commit()
        return result.rowcount > 0
