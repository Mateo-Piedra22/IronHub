...
class GymRepository(BaseRepository):
        ...
        return [
            {
                ...
                "activa": c.activa,
                "tipo_clase_id": c.tipo_clase_id,
            }
            for c in self.db.scalars(stmt).all()
        ]

    def crear_clase(
        self, nombre: str, descripcion: str = None, tipo_clase_id: int = None
    ) -> int:
        clase = Clase(
            nombre=nombre,
            descripcion=descripcion,
    ...
    def obtener_horarios_clase(self, clase_id: int) -> List[Dict]:
        stmt = select(ClaseHorario).where(
            ClaseHorario.clase_id == clase_id, ClaseHorario.activo == True
        )
        return [
            {
                "id": h.id,
    ...
    def inscribir_usuario_clase(self, usuario_id: int, clase_horario_id: int) -> bool:
        ...
        inscritos = self.db.scalar(
            select(func.count(ClaseUsuario.id)).where(
        ...
        if inscritos >= (horario.cupo_maximo or 0):
            posicion = (
                self.db.scalar(
                    select(func.max(ClaseListaEspera.posicion)).where(
                        ClaseListaEspera.clase_horario_id == clase_horario_id
                    )
                )
                or 0
            ) + 1
            wait = ClaseListaEspera(
                usuario_id=usuario_id,
        ...
        inscripcion = ClaseUsuario(
            usuario_id=usuario_id, clase_horario_id=clase_horario_id
        )
        self.db.add(inscripcion)
        self.db.commit()
        return True

    def cancelar_inscripcion_clase(
        self, usuario_id: int, clase_horario_id: int
    ) -> bool:
    ...
    def obtener_inscritos_clase(self, clase_horario_id: int) -> List[Dict]:
        stmt = (
            select(Usuario)
            .join(ClaseUsuario)
            .where(ClaseUsuario.clase_horario_id == clase_horario_id)
        )
        return [
            {"id": u.id, "nombre": u.nombre, "dni": u.dni}
        ...
        total_activos = (
            self.db.scalar(
                select(func.count(Usuario.id)).where(
                    Usuario.activo == True, Usuario.rol.in_(["socio", "miembro"])
                )
            )
            or 0
        )
    ...
    def registrar_ejercicios_batch(
        self,
        ejercicios_items: List[Dict[str, Any]],
        ...
        for item in ejercicios_items:
            ...
            existing = self.db.scalar(
                select(Ejercicio).where(
                    func.lower(Ejercicio.nombre) == str(nombre).lower()
                )
            )

            if existing:
                if not skip_duplicates:
                    existing.grupo_muscular = (
                        item.get("grupo_muscular") or existing.grupo_muscular
                    )
                    existing.descripcion = (
                        item.get("descripcion") or existing.descripcion
                    )
