...
class PaymentRepository(BaseRepository):
    ...
    def obtener_todos_pagos(self) -> List[Dict]:
        stmt = select(Pago).order_by(
            Pago.fecha_pago.desc(), Pago.año.desc(), Pago.mes.desc()
        )
        pagos = self.db.scalars(stmt).all()
        return [
            {
        ...
        return [
            {
                ...
                "icono": m.icono,
                "color": m.color,
                "comision": float(m.comision) if m.comision else 0.0,
                "activo": m.activo,
    ...
    def registrar_pagos_batch(
        self,
        pagos_items: List[Dict[str, Any]],
        skip_duplicates: bool = False,
        validate_data: bool = True,
        auto_crear_metodos_pago: bool = False,
    ) -> Dict[str, Any]:
        result = {"insertados": [], "actualizados": [], "omitidos": [], "count": 0}

        # Cache local de metodos
        metodos_cache = {
            m.nombre.lower(): m for m in self.db.scalars(select(MetodoPago)).all()
        }
        ...
        for item in pagos_items:
            try:
                ...
                existing = self.db.scalar(
                    select(Pago).where(
                        Pago.usuario_id == uid, Pago.mes == mes, Pago.año == año
                    )
                )
                ...
                if existing:
                    ...
                    self.actualizar_fecha_proximo_vencimiento(
                        uid,
                        fecha_pago.date()
    ...
    def actualizar_fecha_proximo_vencimiento(
        self, usuario_id: int, fecha_pago: date = None
    ) -> bool:
        ...
        if user.tipo_cuota:
            tc = self.db.scalar(
                select(TipoCuota).where(TipoCuota.nombre == user.tipo_cuota)
            )
            if tc:
    ...
    def obtener_tipos_cuota_activos(self) -> List[TipoCuota]:
        stmt = (
            select(TipoCuota).where(TipoCuota.activo == True).order_by(TipoCuota.nombre)
        )
        return list(self.db.scalars(stmt).all())
