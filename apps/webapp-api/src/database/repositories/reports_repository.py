...
class ReportsRepository(BaseRepository):
        ...
        total_activos = (
            self.db.scalar(
                select(func.count(Usuario.id)).where(
                    Usuario.activo == True,
                    Usuario.rol.in_(["socio", "miembro", "profesor"]),
                )
            )
            or 0
        )
    ...
    def generar_reporte_automatico_periodo(
        self, tipo_reporte: str, fecha_inicio: date, fecha_fin: date
    ) -> dict:
        ...
        try:
            if tipo_reporte == "usuarios_nuevos":
                stmt = select(
                    ...
                    func.date(Usuario.fecha_registro).between(fecha_inicio, fecha_fin)
                )
    ...
    def obtener_kpis_dashboard(self) -> dict:
        ...
        try:
            ...
            classes_today = (
                self.db.scalar(select(func.count(Clase.id)).where(Clase.activa == True))
                or 0
            )
    ...
    def obtener_estadisticas_base_datos(self) -> dict:
        ...
        try:
            ...
            for name, model in models.items():
                estadisticas[f"total_{name}"] = (
                    self.db.scalar(select(func.count(model.id))) or 0
                )

            # Tamaño BD (Postgres specific)
            try:
                estadisticas["tamaño_bd"] = self.db.scalar(
            ...
            # Conexiones activas
            try:
                estadisticas["conexiones_activas"] = (
                    self.db.scalar(
                        text(
    ...
    def obtener_estadisticas_automatizacion(self) -> dict:
        ...
        try:
            ...
            # Estados activos
            stmt_states = (
                select(UsuarioEstado.estado, func.count(UsuarioEstado.id))
                .where(UsuarioEstado.activo == True)
                .group_by(UsuarioEstado.estado)
            )
            ...
            proximos_vencer = (
                self.db.scalar(
                    select(func.count(UsuarioEstado.id))
                    .join(Usuario)
                    .where(
                        UsuarioEstado.activo == True,
                        UsuarioEstado.fecha_vencimiento.between(
            ...
            try:
                ...
                automatizaciones_mes = (
                    self.db.scalar(
                        select(func.count(HistorialEstado.id)).where(
                            HistorialEstado.fecha_accion
            ...
            return {
                ...
                "alertas": {"proximos_vencer": proximos_vencer, "vencidos": vencidos},
                "automatizacion": {
