...
class AuditRepository(BaseRepository):
    ...
    def obtener_audit_logs(
        ...
        if fecha_inicio:
            stmt = stmt.where(AuditLog.timestamp >= fecha_inicio)
        if fecha_fin:
            stmt = stmt.where(AuditLog.timestamp <= fecha_fin)
    ...
    def obtener_estadisticas_auditoria(self, dias: int = 30) -> Dict:
        ...
        stmt_acciones = (
            ...
            .where(AuditLog.timestamp >= fecha_limite)
            .group_by(AuditLog.action)
            .order_by(func.count(AuditLog.id).desc())
        )
        acciones = [
            {"action": r[0], "count": r[1]}
            for r in self.db.execute(stmt_acciones).all()
        ]

        # Actividad por usuario
        stmt_users = (
            select(Usuario.nombre, func.count(AuditLog.id))
            .join(Usuario)
            .where(AuditLog.timestamp >= fecha_limite)
            .group_by(Usuario.nombre, AuditLog.user_id)
            .order_by(func.count(AuditLog.id).desc())
            .limit(10)
        )
        usuarios = [
            {"nombre": r[0], "count": r[1]} for r in self.db.execute(stmt_users).all()
        ]
        ...
        tablas = [
            {"table_name": r[0], "count": r[1]}
            for r in self.db.execute(stmt_tables).all()
        ]
    ...
    def obtener_diagnosticos(
        ...
        stmt = (
            ...
            .order_by(SystemDiagnostics.timestamp.desc())
            .limit(limit)
        )
        if component:
            stmt = stmt.where(SystemDiagnostics.component == component)
        if status:
            stmt = stmt.where(SystemDiagnostics.status == status)
        if diagnostic_type:
            stmt = stmt.where(SystemDiagnostics.diagnostic_type == diagnostic_type)
