...
class PaymentService(BaseService):
    ...
    def _sync_membership_from_payment(
        self,
        usuario: Usuario,
        *,
            ...
            try:
                self.db.execute(
                    text(
                        """
                        UPDATE memberships
                        SET end_date = GREATEST(COALESCE(end_date, :ed), :ed),
                            plan_name = COALESCE(:plan_name, plan_name),
                            updated_at = NOW()
                        WHERE id = :id
                        """
                    ),
                    {"id": int(mid), "ed": end_date, "plan_name": plan_name},
                )
            except Exception:
                pass
            return
        try:
            self.db.execute(
                text(
                    ...
                    VALUES (:uid, :plan_name, 'active', CURRENT_DATE, :ed, TRUE, NOW(), NOW())
                    """
                ),
                {"uid": int(usuario.id), "plan_name": plan_name, "ed": end_date},
            )
        except Exception:
            pass
    ...
    def modificar_pago_avanzado(
        ...
        try:
            ...
            self.db.execute(delete(PagoDetalle).where(PagoDetalle.pago_id == pago_id))
            ...
            now = (
                fecha_pago
                or getattr(pago, "fecha_pago", None)
                or self._now_local_naive()
            )
    ...
    def actualizar_pago_con_diferencial(
        ...
        try:
            ...
            for detalle in detalles_originales:
                desc = str(getattr(detalle, "descripcion", "") or "").strip()
                tipo_original = self.db.execute(
                    select(TipoCuota).where(func.lower(TipoCuota.nombre) == desc.lower())
                ).scalar_one_or_none()
                if tipo_original and tipo_original.duracion_dias:
                    dias_originales += int(tipo_original.duracion_dias) * int(
            ...
            for item in nuevo_items:
                ...
                tipo_nuevo = self.db.execute(
                    select(TipoCuota).where(
                        func.lower(TipoCuota.nombre) == descripcion.lower()
                    )
                ).scalar_one_or_none()
            ...
            return {
                ...
                "nueva_fecha_vencimiento": str(usuario.fecha_proximo_vencimiento)
    ...
    def verificar_pago_actual(self, usuario_id: int, mes: int, año: int) -> bool:
        """Check if user has paid for given month/year."""
        stmt = select(func.count(Pago.id)).where(
            Pago.usuario_id == usuario_id, Pago.mes == mes, Pago.año == año
        )
    ...
    def registrar_pago_avanzado(
        ...
        try:
            ...
            if pago:
                ...
                try:
                    pago.monto = float(pago.monto or 0) + float(total_batch)
                except Exception:
                ...
                if sucursal_id is not None:
                    try:
                        pago.sucursal_id = int(sucursal_id)
                    except Exception:
                        pass
                if tipo_cuota_id is not None:
                    try:
                        pago.tipo_cuota_id = int(tipo_cuota_id)
                    except Exception:
                        pass
                logger.info(
                    f"Actualizando pago para usuario {usuario_id} (Mes {mes_eff}/{año_eff}): {total_batch}."
                )
            else:
                # CREATE new payment
                pago = Pago(
                    ...
                    tipo_cuota_id=int(tipo_cuota_id) if tipo_cuota_id is not None else None,
                    monto=total_batch,
                    mes=mes_eff,
                    año=año_eff,
            ...
            try:
                ...
                if tipo_cuota_id is not None:
                    try:
                        tc = self.db.get(TipoCuota, int(tipo_cuota_id))
                        if tc and getattr(tc, "nombre", None):
                            plan_name = str(tc.nombre)
                    except Exception:
                        plan_name = None
                if plan_name is None:
                    plan_name = str(getattr(usuario, "tipo_cuota", "") or "").strip() or None
                fpv = getattr(usuario, "fecha_proximo_vencimiento", None)
                ed = None
                if fpv:
                    try:
                        ed = fpv.date() if hasattr(fpv, "date") else fpv
                    except Exception:
                        ed = None
                self._sync_membership_from_payment(usuario, plan_name=plan_name, end_date=ed)
            except Exception:
                pass
            ...
            if idem:
                try:
                    inserted = self.db.execute(
                        text(
                            """
                            INSERT INTO pagos_idempotency (key, pago_id, usuario_id, mes, año, created_at)
                            VALUES (:k, :pid, :uid, :m, :y, NOW())
                    ...
                    if not inserted:
                        ...
                        if existing_pid and int(existing_pid) != int(pago.id):
                            self.db.rollback()
                            return int(existing_pid)
                except Exception:
                    pass
    ...
    def _recalcular_estado_usuario(self, usuario_id: int) -> Dict[str, Any]:
        ...
        if hoy <= primer_vencimiento:
            ...
            ciclos_vencidos = (
                dias_desde_primer_venc + duracion_dias - 1
            ) // duracion_dias
        ...
        if (not exento) and cuotas_vencidas >= 3 and cuotas_previas < 3:
            try:
                ...
                self.desactivar_usuario_por_cuotas_vencidas(
                    int(usuario_id), motivo=motivo
                )
    ...
    def _desactivar_usuario_por_cuotas(
        self, usuario: Usuario, cuotas_vencidas: int
    ) -> None:
    ...
    def _actualizar_estado_usuario_tras_pago(
        self, usuario: Usuario, fecha_pago: datetime
    ) -> None:
        ...
        base = fecha
        try:
            current = getattr(usuario, "fecha_proximo_vencimiento", None)
            ...
            base = fecha
        proximo = base + timedelta(days=int(duracion_dias))
    ...
    def desactivar_usuario_por_cuotas_vencidas(
        self, usuario_id: int, motivo: str = "3 cuotas vencidas"
    ) -> bool:
        """
        ...
        if self._is_exempt_role(getattr(usuario, "rol", None)):
            logger.info(
                f"Usuario {usuario_id} exento de desactivación (rol: {(getattr(usuario, 'rol', None) or '')})"
            )
            return False
        ...
        self.user_repo.registrar_cambio_estado(
            usuario_id, "inactivo", "desactivacion_cuotas", motivo
        )

        self.db.commit()
        try:
            WhatsAppDispatchService(self.db).send_deactivation(
                int(usuario_id), str(motivo or "cuotas vencidas")
            )
        except Exception:
            pass
        return True
    ...
    def procesar_usuarios_morosos_con_whatsapp(
        self, enviar_recordatorios: bool = True, whatsapp_manager=None
    ) -> Dict[str, Any]:
        """
        ...
        for usuario in usuarios_morosos:
            ...
            if resultado.get("desactivado"):
                ...
                if enviar_recordatorios and whatsapp_manager:
                    try:
                        ...
                        errores_envio.append(
                            {"usuario_id": usuario.id, "error": str(e)}
                        )
                ...
                if cuotas > 0:
                    try:
                        whatsapp_manager.enviar_recordatorio_cuota_vencida(
                            usuario_id=usuario.id
                        )
    ...
    def procesar_recordatorios_proximos_vencimientos(
        self, dias_antes: int = 3, whatsapp_manager=None
    ) -> Dict[str, Any]:
        """
        ...
        for usuario in usuarios:
            if whatsapp_manager and usuario.telefono:
                try:
                    ...
                    dias_restantes = (
                        (usuario.fecha_proximo_vencimiento - hoy).days
                        if usuario.fecha_proximo_vencimiento
                        else 0
                    )
                    ...
                    user_data = {
                        ...
                        "due_date": usuario.fecha_proximo_vencimiento.strftime(
                            "%d/%m/%Y"
                        )
                        if usuario.fecha_proximo_vencimiento
                        else "N/A",
                    ...
                    whatsapp_manager.send_overdue_payment_notification(
                        user_data, to=usuario.telefono
                    )
                    recordatorios_enviados += 1
                except Exception as e:
        ...
        return [
            {
                ...
                if u.fecha_proximo_vencimiento
                else None,
    ...
    def obtener_estadisticas_pagos(self, año: Optional[int] = None) -> Dict[str, Any]:
        ...
        stmt = select(func.count(Pago.id), func.sum(Pago.monto)).where(Pago.año == año)
        ...
        # Monthly breakdown
        stmt_mensual = (
            select(Pago.mes, func.count(Pago.id), func.sum(Pago.monto))
            .where(Pago.año == año)
            .group_by(Pago.mes)
        )
        ...
        for row in self.db.execute(stmt_mensual):
            mensual[row[0]] = {"count": row[1], "total": float(row[2] or 0)}
    ...
    def obtener_pagos_por_fecha(
            ...
            try:
                comps = (
                    self.db.execute(
                        ...
                        .order_by(
                            ComprobantePago.pago_id.asc(),
                            ComprobantePago.fecha_creacion.desc(),
                        )
                    )
                    .scalars()
                    .all()
        ...
        metodos = {
            m.id: m.nombre for m in self.db.execute(select(MetodoPago)).scalars().all()
        }
        ...
        return [
            {
                ...
                "metodo_pago": metodos.get(p.metodo_pago_id)
                if p.metodo_pago_id
                else None,
    ...
    def obtener_pagos_por_fecha_paginados(
        ...
        if sucursal_id is not None:
            try:
                sid = int(sucursal_id)
                if sid > 0:
                    stmt = stmt.where(Pago.sucursal_id == sid)
                    count_stmt = count_stmt.where(Pago.sucursal_id == sid)
            except Exception:
                pass
        ...
        try:
            suc_ids = sorted(
                {
                    int(getattr(p, "sucursal_id"))
                    for p in pagos
                    if getattr(p, "sucursal_id", None) is not None
                }
            )
        except Exception:
            suc_ids = []
        if suc_ids:
            try:
                rows = (
                    self.db.execute(
                        select(Sucursal.id, Sucursal.nombre).where(Sucursal.id.in_(suc_ids))
                    )
                    .all()
                )
        ...
        items = [
            {
                ...
                "anio": p.año,
                "fecha_pago": p.fecha_pago.isoformat() if p.fecha_pago else None,
                "sucursal_id": int(p.sucursal_id) if getattr(p, "sucursal_id", None) is not None else None,
                "sucursal_nombre": sucursal_nombre_by_id.get(int(p.sucursal_id)) if getattr(p, "sucursal_id", None) is not None else None,
                "metodo_pago_id": p.metodo_pago_id,
                "metodo_pago": metodos.get(p.metodo_pago_id)
                if p.metodo_pago_id
                else None,
                ...
                "tipo_cuota": getattr(p, "concepto", None)
                or (p.usuario.tipo_cuota if p.usuario else None),
            }
            for p in pagos
        ]
    ...
    def obtener_pago_resumen(self, pago_id: int) -> Optional[Dict[str, Any]]:
        ...
        metodo = (
            self.db.get(MetodoPago, pago.metodo_pago_id)
        ...
        total_detalles = sum(
            float(d.cantidad or 1) * float(d.precio_unitario or 0) for d in detalles
        )
        ...
        return {
            ...
            # tipo_cuota: Use pago's concepto (historical), then fallback to user's tipo_cuota
            "tipo_cuota_nombre": pago.concepto
            or (usuario.tipo_cuota if usuario else None),
            "detalles": [
                {
                    "id": d.id,
                    "pago_id": d.pago_id,
        ...
        try:
            stmt = (
                ...
                .where(
                    ...
                    NumeracionComprobante.tipo_comprobante
                    == self._RECIBO_TIPO_COMPROBANTE,
                )
                .limit(1)
            )
        ...
        try:
            ...
            if not nc:
                nc = NumeracionComprobante(
                    activo=True, tipo_comprobante=self._RECIBO_TIPO_COMPROBANTE
                )
                self.db.add(nc)
    ...
    def increment_receipt_number(self) -> str:
        ...
        try:
            ...
            num_str = str(current).zfill(padding)
            formatted = (
                f"{prefix}{separador}{num_str}" if separador else f"{prefix}{num_str}"
            )
    ...
    def obtener_profesor_nombre(
        self, profesor_user_id: Optional[int] = None, profesor_id: Optional[int] = None
    ) -> Optional[str]:
