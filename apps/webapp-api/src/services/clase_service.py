...
from src.database.orm_models import (
    Clase, ClaseBloque, ClaseBloqueItem, Usuario
)
...
class ClaseService(BaseService):
    ...
    def obtener_clases(self) -> List[Dict[str, Any]]:
        """Get all classes."""
        try:
            clases = self.db.scalars(select(Clase).order_by(Clase.nombre)).all()
            return [
                {
                    'id': c.id,
                    'nombre': c.nombre,
    ...
    def crear_clase(self, data: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            if hasattr(Clase, 'capacidad'): clase.capacidad = data.get('capacidad')
            if hasattr(Clase, 'color'): clase.color = data.get('color', '#3498db')
            if hasattr(Clase, 'icono'): clase.icono = data.get('icono')
            if hasattr(Clase, 'duracion_minutos'): clase.duracion_minutos = data.get('duracion_minutos', 60)
    ...
    def actualizar_clase(self, clase_id: int, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'descripcion' in data: clase.descripcion = data['descripcion']
            if 'activo' in data: clase.activa = data['activo']
    ...
    def actualizar_clase_bloque(self, bloque_id: int, nombre: str, items: List[Dict[str, Any]]) -> bool:
            ...
            return [
                {
                    ...
                    'nombre_ejercicio': i.ejercicio.nombre if i.ejercicio else 'Unknown',
                    'orden': i.orden,
                    'series': i.series,
