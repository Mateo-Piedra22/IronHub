...
class AuditService(BaseService):
    ...
    def log_from_request(
        ...
        try:
            ...
            session_id = None
            try:
                user_id = request.session.get("user_id") or request.session.get("gestion_profesor_user_id")
                if user_id:
                    user_id = int(user_id)
                # Generate a session identifier from session keys (for debugging)
                session_id = str(hash(frozenset(request.session.keys())))[:16] if request.session else None
            except Exception:
                pass
