...
class UserService(BaseService):
    def __init__(self, db: Session = None):
        ...
        self.user_repo = UserRepository(
            self.db, None, None
        )  # Logger and Cache can be None for now or injected
    ...
    def create_user(self, data: Dict[str, Any], is_owner: bool = False) -> int:
        ...
        if "tipo_cuota_id" in data and data["tipo_cuota_id"] is not None:
            try:
                tc = self.payment_repo.obtener_tipo_cuota_por_id(
                    int(data["tipo_cuota_id"])
                )
                if tc:
                    ...
                    try:
                        ...
                        data["fecha_proximo_vencimiento"] = (
                            self._today_local_date() + timedelta(days=duracion)
                        )
                    except Exception:
                        pass
            except Exception:
    ...
    def update_user(
        ...
        if self._is_privileged_role(getattr(current_user, "rol", None)) and not bool(
            is_owner
        ):
            raise PermissionError(
                "Solo el dueño puede modificar un usuario privilegiado"
            )

        if (
            "rol" in data
            and self._is_privileged_role(data.get("rol"))
            and not bool(is_owner)
        ):
            raise PermissionError("Solo el dueño puede asignar roles privilegiados")
        ...
        if should_recalc_expiry and "fecha_proximo_vencimiento" not in data:
            try:
                ...
                if tc_name:
                    ...
                    current_user.fecha_proximo_vencimiento = (
                        self._today_local_date() + timedelta(days=duracion)
                    )
            except Exception:
                pass
    ...
    def set_user_pin(
        self, user_id: int, new_pin: str, *, is_owner: bool = False
    ) -> bool:
    ...
    def get_user_panel_data(self, user_id: int) -> Dict[str, Any]:
        ...
        rutinas = (
            [r for r in u.rutinas if r.activa]
            if hasattr(u, "rutinas") and u.rutinas
            else []
        )
    ...
    def add_user_tag(
        self, user_id: int, tag_data: Dict[str, Any], assigned_by: Optional[int]
    ):
    ...
    def add_user_state(
        self, user_id: int, data: Dict[str, Any], creado_por: int = None
    ) -> int:
