...
class AdminService(BaseService):
    ...
    def cambiar_contrasena_dueno(self, current: str, new: str) -> Dict[str, Any]:
        ...
        try:
            ...
            row = self.db.execute(
                text(
                    "SELECT id, pin FROM usuarios WHERE rol IN ('dueno', 'owner', 'admin') ORDER BY id LIMIT 1"
                )
            ).fetchone()
            if not row:
            ...
            if stored.startswith("$2"):
                try:
                    valid = bcrypt.checkpw(current.encode(), stored.encode())
                except:
                    valid = False
            else:
            ...
            try:
                ...
                if tenant:
                    ...
                    try:
                        ...
                        admin_db.execute(
                            text(
                                "UPDATE gyms SET owner_password_hash = :hash WHERE subdominio = :sub"
                            ),
                            {"hash": new_hash, "sub": tenant},
                        )
                        admin_db.commit()
                        logger.info(
                            f"Synced owner password for tenant '{tenant}' to Admin DB."
                        )
    ...
    def secure_owner(self) -> Dict[str, Any]:
        ...
        try:
            row = self.db.execute(
                text(
                    "SELECT id, nombre, pin, rol FROM usuarios WHERE rol IN ('dueno', 'owner', 'admin') ORDER BY id LIMIT 1"
                )
            ).fetchone()
            actions = []
            ...
            if not row:
                result = self.db.execute(
                    text(
                        "INSERT INTO usuarios (nombre, rol, activo, pin) VALUES ('Administrador', 'dueno', TRUE, '1234') RETURNING id"
                    )
                )
                owner_id = result.fetchone()[0]
                actions.append(f"Created owner with ID {owner_id}")
                row = self.db.execute(
                    text("SELECT id, nombre, pin, rol FROM usuarios WHERE id = :id"),
                    {"id": owner_id},
                ).fetchone()
    ...
    def renumerar_usuarios(self, start_id: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            self.db.execute(
                text("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS _new_id INTEGER")
            )

            for old, nid in mapping.items():
                self.db.execute(
                    text("UPDATE usuarios SET _new_id = :nid WHERE id = :old"),
                    {"nid": nid, "old": old},
                )
            self.db.execute(
                text("UPDATE usuarios SET _new_id = id WHERE _new_id IS NULL")
            )
            ...
            for table, col in related:
                try:
                    for old, nid in mapping.items():
                        self.db.execute(
                            text(
                                f"UPDATE {table} SET {col} = :temp WHERE {col} = :old"
                            ),
                            {"temp": nid + 1000000, "old": old},
                        )
                    for old, nid in mapping.items():
                        self.db.execute(
                            text(
                                f"UPDATE {table} SET {col} = :nid WHERE {col} = :temp"
                            ),
                            {"nid": nid, "temp": nid + 1000000},
                        )
                except:
                    pass
            ...
            self.db.execute(
                text(
                    "UPDATE usuarios SET id = _new_id WHERE _new_id IS NOT NULL AND _new_id != id"
                )
            )
